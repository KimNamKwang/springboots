...
<html lang="en">
  ...
  <body>
    <h3>JSTL Formats</h3>
    <div>234123.12 --> 234,124.12</div>
    <div>2023.01.16 10:18 --> 2023 1 16</div>
    <hr />
    <h3>JSTL Formats - numbers</h3>
    <c:set var="num_first" value="1234567.89" />
    <div>
      세자리 구분 : <fmt:formatNumber value="${num_first}" type="number" />
    </div>
    <c:set var="num_second" value="0.1234" />
    <div>
      백분율 : <fmt:formatNumber value="${num_second}" type="percent" />
    </div>
    <hr />
    <h3>JSTL Formats - Date</h3>
    ...
    <div>
      date and time 출력 : <fmt:formatDate value="${date_first}" type="both" />
    </div>

    <hr />
    <h3>JSTL Formats - Location</h3>
    ...
    <div>
      한국 :
      <fmt:setLocale value="ko-KR" scope="session" /> <%=response.getLocale() %>
    </div>
    ...
    <div>
      미국 :
      <fmt:setLocale value="en-US" scope="session" /> <%=response.getLocale() %>
    </div>
    <div>통화 : <fmt:formatNumber value="${num_third}" type="currency" /></div>
    <div>날짜 : <fmt:formatDate value="${date_third}" type="both" /></div>
    ...
    <div>
      프랑스 :
      <fmt:setLocale value="fr-FR" scope="session" /><%=response.getLocale() %>
    </div>
    <div>통화 : <fmt:formatNumber value="${num_third}" type="currency" /></div>
    <div>날짜 : <fmt:formatDate value="${date_third}" type="both" /></div>
  </body>
